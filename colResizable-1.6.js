...
(function($){
	...
	var init = function( tb, options){
		...
		// if(!(tb.style.width || tb.width)) t.outerWidth(t.outerWidth()); //I am not an IE fan at all, but it is a pity that only IE has the currentStyle attribute working as expected. For this reason I can not check easily if the table has an explicit width or if it is rendered as "auto"
	...
	var syncGrips = function (t){
		...
		t.outerWidth(t.w);               //set last correct table width
	...
	var onGripMouseDown = function(e){
		...
		if(t.c[o.i].l) for(var i=0,c; i<t.ln; i++){ c=t.c[i]; c.l = false; c.w= c.outerWidth(); } 	//if the colum is locked (after browser resize), then c.w must be updated
